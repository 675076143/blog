...
先澄清一个容易误判的细节：原脚本虽然写了删除并重建 namespace 的命令，但流水线使用的 ServiceAccount 没有相应权限。两条命令实际都返回了 `Forbidden`，错误又被 `|| echo` 吞掉，因此 namespace 从未被删除，原来的发布本身就是在现有 Deployment 上执行原地滚动更新，并没有因为重建 namespace 而停机。

所以这部分改造不能算作“从停机发布改成滚动发布”。真正的收益是删除反复失败且制造噪声的无效命令，明确复用已有 namespace，并为原本已经存在的滚动更新补上完成状态验收。
...
7. **先核对日志，再评价旧脚本。** 写了 namespace 删除命令，不代表它真的执行成功；被吞掉的 RBAC 错误会改变对发布行为的判断。
8. **`kubectl apply` 不是发布完成。** rollout 成功才是。
9. **性能指标不能脱离语义。** 一个 13 秒结束但 Pod 起不来的部署 job，没有比 48 秒的可靠部署更快。
